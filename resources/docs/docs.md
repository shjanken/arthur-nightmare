...
The `home-routes` handler in the `arthur-nightmare.routes.home` namespace
...
The `home-page` function will in turn call the `arthur-nightmare.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `arthur-nightmare.handler` namespace:
...
Request middleware functions are located under the `arthur-nightmare.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `arthur-nightmare.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `arthur-nightmare.db.core` in some other namespace.
